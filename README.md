...
* rtrt 是**真实发生的奖励**，但在 LLM 生成过程中，**只有完整回复结束后才有非零值**（例如人类打分 rT=5rT=5）；在中间步骤（如生成“今”“天”时），rt=0rt=0；
...
> 注：LxLx 是 prompt 长度，TT 是生成回复长度（实际中常 padding 到固定 max\_len）。
